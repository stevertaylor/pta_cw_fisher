...
def get_delay_batch(pars_vec, pulsar_batch, pterm=True, phase_free=False):
    """
    ...
    # 4. Evolution setup
    # toas: (N_psrs, N_toas) - assume tref=0 for simplicity update if needed
    toas = pulsar_batch.toas 
    w0 = jnp.pi * fgw
    phase0 = cw_phase0 / 2
    ...
    if pterm:
        ...
        if phase_free:
            ...
            # Calculate omega_p0 for phase shift
            # Note: omega_p is (N_psrs, N_toas), we want t=0 approx or first element?
            # Usually omega_p varies slowly. Let's use the first TOA or recompute at tp[0]
            # Recomputing purely:
            # tp0 = -p_dist * (1-cosMu) # at t=0
            # omega_p0_val, _ = evolve(tp0)
            
            # Simplified: use the evolved array
            omega_p0 = omega_p[:, 0:1] 
...
@partial(jax.jit, static_argnums=(2, 3))
def computer_snr2_batch(pars_vec, pulsar_batch, pterm=True, phase_free=False):
    """Compute combined SNR^2 for the entire batch"""
    # Get all residuals: (N_psrs, N_toas)
    res = get_delay_batch(pars_vec, pulsar_batch, pterm, phase_free)
...
@partial(jax.jit, static_argnums=(2, 3))
def compute_total_fisher(pars_vec, pulsar_batch, pterm=True, phase_free=False):
    """
    ...
    def model_fn(p):
        return get_delay_batch(p, pulsar_batch, pterm, phase_free)
...
@jax.jit
def compute_pulsar_phases_batch(pars_vec, pulsar_batch):
    """
    ...
    Returns:
        phases: Shape (N_psrs,) linked phases at t=0 (relative to phase0).
    """
    cw_costheta, cw_phi, cw_cosinc, cw_log10_Mc, cw_log10_fgw, cw_log10_dist, cw_phase0, cw_psi = pars_vec[:8]
    n_psrs = pulsar_batch.pos.shape[0]
